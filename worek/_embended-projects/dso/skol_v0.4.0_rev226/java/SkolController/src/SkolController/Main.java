...
public class Main 
{
    ...
    //public static int[] streamData = null; 
    ...
    // these both are to adjust the x & y Offset ranges
    // lower value means finder adjusting, but lesser range
...
//                {
//                    logger.logError("Received Response, but did not wait for any.. (dropped) " + hexUtil.byteToHexString(myMessage.opCode));
//                    bytesToWaitFor.clear();
//                    return;
//                }
//                else if(bytesToWaitFor.contains(myMessage.opCode))
//                {
//                    if(bytesToWaitFor.get(0) != myMessage.opCode)
//                    {
//                        logger.logError("Received old Response (dropped) " + hexUtil.byteToHexString(myMessage.opCode));
//                        bytesToWaitFor.clear();
//                        return;
//                    }
//                    else
...
//                    }
//                }
//                else if(bytesToWaitFor.size() != 1 )
//                {
//                   logger.logError("bytesToWaitFor has too many entries - should never happen  (dropped) " + hexUtil.byteToHexString(myMessage.opCode));
//                    bytesToWaitFor.clear();
//                    return;
//
//                }
//                else
//                {
//                    logger.logError("Received Response, but did not wait for this (dropped and dropped all waiting) " + hexUtil.byteToHexString(myMessage.opCode));
//                    bytesToWaitFor.clear();
//                    return;
//                }
//                break;
...
//        }
        ...
        switch(myMessage.opCode)
        {
            ...
            case opCodes.singleVoltValueCh1:
                ...
                break;
            case opCodes.getTimeDividerCh0:
                if(hexUtil.byteToUnsignedInt(myMessage.dataByte[0]) == timeDividerIntegerCh0)
                    logger.logInfo("Set TimeDivider to " + myTimeDividerHelper.getItem(timeDividerIntegerCh0) + " succcess! (CH0)");
                else
                    logger.logError("Set TimeDivider to " + myTimeDividerHelper.getItem(timeDividerIntegerCh0) + " ERROR! (CH0)");
                bytesToWaitFor.clear();
                break;
            case opCodes.getTimeDividerCh1:
                if(hexUtil.byteToUnsignedInt(myMessage.dataByte[0]) == timeDividerIntegerCh1)
                    logger.logInfo("Set TimeDivider to " + myTimeDividerHelper.getItem(timeDividerIntegerCh1) + " succcess! (CH1)");
                else
                    logger.logError("Set TimeDivider to " + myTimeDividerHelper.getItem(timeDividerIntegerCh1) + " ERROR! (CH1)");
                bytesToWaitFor.clear();
                break;
            case opCodes.getTriggerMode: // B0           
                if(myMessage.dataByte[0] == constants.TriggerOff && (triggerOn == false))
                    logger.logInfo("Set Trigger Off succcess!");
                else if(myMessage.dataByte[0] == constants.TriggerNegEdge && (triggerAtPosEdge == false))
                    logger.logInfo("Set TriggerEdge to NegEdge succcess!");
                else if(myMessage.dataByte[0] == constants.TriggerPosEdge && (triggerAtPosEdge == true))
                    logger.logInfo("Set TriggerEdge to PosEdge succcess!");
                else 
                    logger.logInfo("Set TriggerMode ERROR! dataByte" + hexUtil.byteToHexString(myMessage.dataByte[0]));
                bytesToWaitFor.clear();
                break;
            case opCodes.getTriggerLevel:
                if(myMessage.dataByte[1] == triggerLevelHighByte && myMessage.dataByte[0] == triggerLevelLowByte)
                    logger.logInfo("Set TriggerLevel succcess!");
                else
                    logger.logError("Set TriggerLevel ERROR!");
                bytesToWaitFor.clear();
                break;
            case opCodes.getSamplesPerFrameMode:
                if(myMessage.dataByte[0] == constants.SamplesPerFrameMode[myGui.jComboBox_Resolution.getSelectedIndex()])
                    logger.logInfo("Set Resolution succcess!");
                else
                    logger.logError("Set Resolution ERROR!");
                bytesToWaitFor.clear();
                break;
//            case opCodes.getTriggerLevelLowByte:
//                if(myMessage.dataByte[0] == triggerLevelLowByte)
//                    logger.logInfo("Set TriggerLevel succcess (2/2)!");
//                else
//                    logger.logError("Set TriggerLevel ERROR (2/2 triggerLevelLowByte)!");
//                bytesToWaitFor.clear();
//                break;
            default:
...
//        try 
//        {
//            message m = new message(opCodes.stopDataAqu, incrFlowCount(), new byte[]{}, (byte) 1);
//            communicator.sendMessage(m.rawMessage);
//        } 
//        catch (Exception ex) 
//        {
//            logger.logError(" generating msg in stopAquisition", ex);
//        }
    }
    ...
    public static void setDeviceTriggerLevel(int newLevel)
    {
        ...
        if(newLevel != triggerLevel)
        {
            ...
            try 
            {
                message setTriggerLevelHighByte = new message(opCodes.setTriggerLevelHighByte, 
                                 new byte[]{newSettings[0]}, (byte) 1);
                communicator.sendMessage(setTriggerLevelHighByte.rawMessage);
                
                
                message setTriggerLevelLowByte = new message(opCodes.setTriggerLevelLowByte, 
                                 new byte[]{newSettings[1]}, (byte) 1);
...
//        }
    ...
    private static void receivedDataStream(int channel, message msg) 
    {
        ...
        if(myFileWriterThread != null)
            myFileWriterThread.writeToFile(hexUtil.byteToHexString(msg.rawMessage));
        
        if(channel == 0 && !myGui.jCheckBox_channel0DisplayOrNot.isSelected())
            return;
        ...
        if(xCoordinates == null)
        {
            ...
            for(int i= 0; i < data.length; i++)
                xCoordinates[i] = ((double)i/dataLenMultiplier) - 5.0;
        }
            ...
            synchronized(Main.yValuesCh0)
            {
                ...
                for(int i= 0; i < data.length; i++)
                {
                    ...
                    if(position >= 0 && position < data.length)
                        yValuesCh0[position] = ((double)data[i] / (double)2048);
                }
            }
            java.awt.EventQueue.invokeLater(
                    new Runnable(){ public void run() { myGui.updateDataCh0(); }});
        }
        else if(channel == 1)
        {
            int positionDiff = 0;
            ...
            synchronized(Main.yValuesCh1)
            {
                ...
                for(int i= 0; i < data.length; i++)
                {
                    position = i + positionDiff;
                    if(position >= 0 && position < data.length)
                        yValuesCh1[position] = ((double)data[i] / (double)2048);
                }
                if(AdcCounterReceivedInChannel1)
                {
                    for(int j= 0; j < data.length; j++)
                        logger.logDebug("AdcCounterReceivedInChannel1 " +  j + "\t"  + data[j]+ " Cycles for single Aquisition");
                }
            }
            java.awt.EventQueue.invokeLater(
                    new Runnable(){ public void run() { myGui.updateDataCh1(); }});
        }
        ...
        if(checkUndersampling)
        {
            ...
            {
                if(myUnderSamplingCheckerThread.dataArray.length != data.length)
                    myUnderSamplingCheckerThread.dataArray = new int[data.length];
                for(int i= 0; i < data.length; i++)
                    myUnderSamplingCheckerThread.dataArray[i] = data[i];
        ...
        if(channel == 1  && keepAquisitionRunning == true)
            doAquisition();
    ...
    private static void readXmlParamFile()
    {
        try {
            ...
            for(int i = 0 ; i < itemCount ; i ++)
            {
                String propertyName;
                if(i < 10)
                    propertyName = "defaultSamplesPerFrameMode0" + i;
                else
                    propertyName = "defaultSamplesPerFrameMode" + i;
